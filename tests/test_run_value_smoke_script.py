...
def test_value_smoke_script_is_valid_codex_only_execution_smoke() -> None:
    ...
    assert "value smoke requires TYPESAFE_API_KEY" in text
    assert 'export AGENT_WORKFLOW_TYPESAFE_API_CALL_LOG="${ROOT}/typesafe-api-audit.jsonl"' in text
    assert 'chmod 600 "${AGENT_WORKFLOW_TYPESAFE_API_CALL_LOG}"' in text
    assert "TypeSafe audit:" in text
    assert "require_decision_runtime_ready" in text
    assert "decision_policy.mode='comparative'" in text
