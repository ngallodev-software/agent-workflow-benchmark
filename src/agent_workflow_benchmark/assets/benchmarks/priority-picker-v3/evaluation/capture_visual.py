...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            # Restore the populated default state before BM5 product/styling checks.
            page.locator('[data-testid="search-input"]').fill("")
            if page.locator('[data-testid="status-filter"]').count():
                page.locator('[data-testid="status-filter"]').select_option("all")
            if page.locator('[data-testid="risk-filter"]').count():
                page.locator('[data-testid="risk-filter"]').select_option("all")
            page.wait_for_timeout(75)

            def factor_descriptions() -> tuple[bool, str]:
                helpers = page.locator('[data-testid="factor-help"]')
                count = helpers.count()
                factors = helpers.evaluate_all(
                    "nodes => nodes.map(node => node.dataset.factor)"
                ) if count else []
                required = {"impact", "urgency", "effort", "confidence", "risk"}
                focus_ok: dict[str, bool] = {}
                hover_ok: dict[str, bool] = {}
                texts: dict[str, str] = {}
                for factor in sorted(required):
                    trigger = page.locator(
                        f'[data-testid="factor-help"][data-factor="{factor}"]'
                    ).first
                    if trigger.count() != 1:
                        focus_ok[factor] = False
                        hover_ok[factor] = False
                        continue
                    trigger.focus()
                    page.wait_for_timeout(40)
                    tooltip = page.locator(
                        '[data-testid="factor-tooltip"][role="tooltip"]:visible'
                    )
                    focus_text = tooltip.last.inner_text().strip() if tooltip.count() else ""
                    focus_ok[factor] = len(focus_text) >= 8
                    trigger.hover()
                    page.wait_for_timeout(40)
                    tooltip = page.locator(
                        '[data-testid="factor-tooltip"][role="tooltip"]:visible'
                    )
                    hover_text = tooltip.last.inner_text().strip() if tooltip.count() else ""
                    hover_ok[factor] = len(hover_text) >= 8
                    texts[factor] = hover_text or focus_text
                passed = (
                    required.issubset(set(factors))
                    and all(focus_ok.get(name, False) for name in required)
                    and all(hover_ok.get(name, False) for name in required)
                )
                observations["factor_descriptions"] = {
                    "count": count,
                    "factors": factors,
                    "focus_ok": focus_ok,
                    "hover_ok": hover_ok,
                ...
                return passed, (
                    f"factor_count={count}; factors={factors}; "
                    f"focus_ok={focus_ok}; hover_ok={hover_ok}"
                )

            checks.append(
                solution_check("ui.factor-descriptions", factor_descriptions)
            )

            def visual_hierarchy() -> tuple[bool, str]:
                items = page.locator('[data-testid="priority-item"]')
                item_count = items.count()
                summary_count = page.locator('[data-testid="summary-strip"]').count()
                control_count = page.locator('[data-testid="control-panel"]').count()
                score_count = page.locator('[data-testid="priority-score"]').count()
                top_flags = (
                    items.evaluate_all(
                        "nodes => nodes.map(node => node.dataset.priorityTier || '')"
                    )
                    if item_count
                    else []
                )
                style_signatures = (
                    items.evaluate_all(
                        """nodes => nodes.map(node => {
                            ...
                            return [s.backgroundColor, s.borderColor, s.borderLeftColor,
                                    s.borderLeftWidth, s.boxShadow, s.fontWeight].join('|');
                        })"""
                    )
                    if item_count
                    else []
                ...
                top_style_distinct = (
                    item_count >= 4
                    and len(set(style_signatures[:3])) >= 1
                    and any(sig != style_signatures[3] for sig in style_signatures[:3])
                )
                ...
                if item_count:
                    ...
                    first.click()
                    page.wait_for_timeout(40)
                    selected_attr = first.get_attribute("aria-selected")
                    first_style = first.evaluate(
                        """node => {
                            const s = getComputedStyle(node);
                            return [s.backgroundColor, s.borderColor, s.boxShadow].join('|');
                        }"""
                    )
                    comparison = (
                        items.nth(3).evaluate(
                            """node => {
                                const s = getComputedStyle(node);
                                return [s.backgroundColor, s.borderColor, s.boxShadow].join('|');
                            }"""
                    ...
                    selected_visible = selected_attr == "true" and first_style != comparison
                passed = (
                    summary_count == 1
                    and control_count == 1
                    and item_count >= 4
                    and score_count >= item_count
                    and top_flags[:3] == ["top", "top", "top"]
                    and top_style_distinct
                    and selected_visible
                )
                observations["visual_hierarchy"] = {
                    "summary_count": summary_count,
                    "control_count": control_count,
                    "item_count": item_count,
                    "score_count": score_count,
                    "top_flags": top_flags,
                    "top_style_distinct": top_style_distinct,
                    "selected_visible": selected_visible,
                }
                return passed, (
                    f"summary={summary_count}; controls={control_count}; "
                    f"items={item_count}; scores={score_count}; top_flags={top_flags[:4]}; "
                    f"top_style_distinct={top_style_distinct}; "
                    f"selected_visible={selected_visible}"
                )

            checks.append(solution_check("ui.visual-hierarchy", visual_hierarchy))

            def status_styling() -> tuple[bool, str]:
                badges = page.locator('[data-testid="status-badge"]')
                count = badges.count()
                rows = (
                    badges.evaluate_all(
                        """nodes => nodes.map(node => {
                            const s = getComputedStyle(node);
                            return {
                                status: node.dataset.status || node.textContent.trim().toLowerCase(),
                                style: [s.backgroundColor, s.color, s.borderColor].join('|')
                            };
                        })"""
                    )
                    if count
                ...
                for row in rows:
                    by_status.setdefault(str(row["status"]), set()).add(str(row["style"]))
                statuses = sorted(by_status)
                styles = {next(iter(values)) for values in by_status.values() if values}
                passed = len(statuses) >= 3 and len(styles) >= 3
                observations["status_styling"] = {
                    "badge_count": count,
                    "statuses": statuses,
                    "distinct_style_count": len(styles),
                }
                return passed, (
                    f"badge_count={count}; statuses={statuses}; "
                    f"distinct_styles={len(styles)}"
                )

            checks.append(solution_check("ui.status-styling", status_styling))

            def export_feedback() -> tuple[bool, str]:
                status = page.locator('[data-testid="export-status"]')
                visible = status.count() == 1 and status.is_visible()
                text = status.inner_text().strip() if visible else ""
                passed = visible and "json" in text.lower() and any(
                    term in text.lower() for term in ("export", "download", "saved")
                )
                observations["export_feedback"] = {
                    "visible": visible,
                    "text": text,
                }
                return passed, f"visible={visible}; text={text!r}"

            checks.append(solution_check("ui.export-feedback", export_feedback))

            def debug_observability() -> tuple[bool, str]:
                toggle = page.locator('[data-testid="debug-toggle"]')
                panel = page.locator('[data-testid="debug-panel"]')
                if toggle.count() != 1 or panel.count() != 1:
                    observations["debug"] = {
                        "toggle_count": toggle.count(),
                        "panel_count": panel.count(),
                    }
                    return False, (
                        f"toggle_count={toggle.count()}; panel_count={panel.count()}"
                    )
                hidden_default = not panel.is_visible()
                before_ids = page.locator('[data-testid="priority-item"]').evaluate_all(
                    "nodes => nodes.map(node => node.dataset.id)"
                )
                toggle.click()
                page.wait_for_timeout(75)
                ...
                bound = page.locator('[data-testid="debug-bound-controls"]')
                bound_text = bound.inner_text().lower() if bound.count() == 1 else ""
                required_controls = {
                    "search-input", "status-filter", "risk-filter", "sort-control",
                    "export-button", "priority-item", "debug-toggle",
                }
                bound_ok = all(name in bound_text for name in required_controls)

                request = page.locator('[data-testid="debug-data-request"]')
                request_data = request.evaluate(
                    """node => ({
                        source: node.dataset.source || '',
                        status: node.dataset.status || '',
                        requestCount: node.dataset.requestCount || '',
                        itemCount: node.dataset.itemCount || ''
                    })"""
                ) if request.count() == 1 else {}
                try:
                    request_count = int(request_data.get("requestCount", "0"))
                    debug_item_count = int(request_data.get("itemCount", "-1"))
                except (TypeError, ValueError):
                    request_count = 0
                    debug_item_count = -1
                request_ok = (
                    bool(request_data.get("source"))
                    and str(request_data.get("status", "")).lower()
                    in {"200", "ok", "loaded", "success"}
                    and request_count >= 1
                    and debug_item_count == len(before_ids)
                )

                state = page.locator('[data-testid="debug-state"]')
                state_before = state.evaluate(
                    """node => ({
                        search: node.dataset.search ?? null,
                        statusFilter: node.dataset.statusFilter ?? null,
                        riskFilter: node.dataset.riskFilter ?? null,
                        sort: node.dataset.sort ?? null,
                        selectedId: node.dataset.selectedId ?? null
                    })"""
                ) if state.count() == 1 else {}
                page.locator('[data-testid="search-input"]').fill("checkout")
                page.wait_for_timeout(75)
                state_after = state.evaluate(
                    """node => ({
                        search: node.dataset.search ?? null,
                        statusFilter: node.dataset.statusFilter ?? null,
                        riskFilter: node.dataset.riskFilter ?? null,
                ...
                state_updates = (
                    state_before.get("search") == ""
                    and state_after.get("search") == "checkout"
                    and state_after.get("statusFilter") is not None
                    and state_after.get("riskFilter") is not None
                    and bool(state_after.get("sort"))
                )
                page.locator('[data-testid="search-input"]').fill("")
                page.wait_for_timeout(40)

                errors = page.locator('[data-testid="debug-errors"]')
                errors_data = errors.evaluate(
                    """node => ({
                        count: node.dataset.count ?? null,
                        text: node.textContent.trim()
                    })"""
                ) if errors.count() == 1 else {}
                try:
                    error_count = int(errors_data.get("count"))
                    errors_ok = error_count >= 0 and bool(errors_data.get("text"))
                except (TypeError, ValueError):
                    error_count = -1
                    errors_ok = False

                after_ids = page.locator('[data-testid="priority-item"]').evaluate_all(
                    "nodes => nodes.map(node => node.dataset.id)"
                )
                non_destructive = before_ids == after_ids
                passed = (
                    hidden_default and visible and bound_ok and request_ok
                    and state_updates and errors_ok and non_destructive
                )
                observations["debug"] = {
                    "hidden_by_default": hidden_default,
                    "visible_when_enabled": visible,
                    "bound_controls_ok": bound_ok,
                ...
                return passed, (
                    f"hidden_default={hidden_default}; visible={visible}; "
                    f"bound_ok={bound_ok}; request_ok={request_ok}; "
                    f"state_updates={state_updates}; errors_ok={errors_ok}; "
                    f"non_destructive={non_destructive}"
                )

            checks.append(
                solution_check("ui.debug-observability", debug_observability)
            )
            browser.close()
