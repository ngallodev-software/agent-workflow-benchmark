...
if [[ ! -f "$QUALIFICATION" ]]; then
  ...
  bash "$SCRIPT_DIR/p0a-qualify.sh"
...
echo
bash "$SCRIPT_DIR/p0b-run-ab.sh"

echo
bash "$SCRIPT_DIR/p0b-validate-ab.sh"

echo
bash "$SCRIPT_DIR/p0b-compute-disputes.sh"
...
echo
bash "$SCRIPT_DIR/p0b-validate-oracle.sh"
