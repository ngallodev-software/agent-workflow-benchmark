...
REPETITIONS="${BM5_REPETITIONS:-1}"
ROOT_ARG=""
EVIDENCE_ARG=""
RESULTS_REPO_ARG="${BM5_RESULTS_REPO:-}"

usage() {
  cat <<'USAGE'
Usage: scripts/run-bm5-slimmed.sh [options]

Run the BM5 steering-first simplification study:
  Priority Picker v3
  structured-direct/v1 vs agent-workflow-bm5/v1
  both arms: gpt-6-luna, high reasoning effort

The BM5 candidate uses agent finish, host-owned acceptance verification,
steering-first communication, per-command/cache telemetry, and conditionally
skips the separate verify/repair model phase after green implementation
acceptance. The structured-direct control still receives the same complete v3
task and all three canonical phase prompts.

Options:
  --root PATH          durable BM5 artifact root (default: BM5_ROOT or mktemp)
  --repetitions N      paired repetitions (default: BM5_REPETITIONS or 1)
...
The script exports Priority Picker v3, creates the fixture, verifies readiness,
runs paired execution, captures visual/debug evidence, emits both official
machine_score and end-to-end-product/v2 product_score, writes a descriptive
report, summarizes granular timing/protocol/TypeSafe evidence, creates the
private evidence archive, and copies sanitized artifacts to benchmark-results/bm5.
Development results do not establish a generalized winner.
USAGE
}
...
if [[ ! "$REPETITIONS" =~ ^[1-9][0-9]*$ ]]; then
  echo "BM5_REPETITIONS must be a positive integer; observed: $REPETITIONS" >&2
  exit 2
fi
...
[[ -n "${TYPESAFE_API_KEY:-}" ]] || {
  echo "BM5 requires TYPESAFE_API_KEY because the benchmark runtime uses comparative mode" >&2
  exit 1
}
...
for name, expected in required.items():
    ...
    if observed != expected:
        raise SystemExit(f"{name}={observed}; BM5 requires {expected}")
settings = load_settings()
if settings.decision_mode != "comparative":
    raise SystemExit(
        f"BM5 requires decision_policy.mode='comparative'; observed {settings.decision_mode!r}"
    )
status = require_decision_runtime_ready(settings)
if status.get("ready") is not True:
    raise SystemExit(f"semantic runtime is not ready: {status}")
print(
    "BM5 semantic preflight: "
    f"mode={status['mode']}; typesafe_api_key=configured; comparative_eval=compatible"
)
PY

ROOT="${ROOT_ARG:-${BM5_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/agent-workflow-bm5.XXXXXX")}}"
SUITE="$ROOT/suite"
...
SUMMARY_JSON="$ROOT/bm5-summary.json"
SEMANTIC_QUALIFICATION_JSON="$ROOT/typesafe-semantic-qualification.json"
EVIDENCE_ARCHIVE="${EVIDENCE_ARG:-${BM5_EVIDENCE_ARCHIVE:-${ROOT}-evidence.tar.gz}}"
...
"$AW_BIN" benchmark bm5-export "$SUITE" --agent-class "$AGENT_CLASS"
...
if executor.get("model") != "gpt-6-luna" or executor.get("effort") != "high":
    raise SystemExit(
        f"BM5 requires gpt-6-luna/high; observed model={executor.get('model')!r} effort={executor.get('effort')!r}"
    )
...
for phase in spec["phases"]:
    ...
    advice = advise_routing_with_policy(
        ...
        task_text=text,
        source_ref=f"bm5:{spec['benchmark_id']}:{phase['id']}",
    )
    contexts.append({
        "phase_id": phase["id"],
        "phase_name": phase["name"],
...
after = audit_count()
value = {
    "schema": "agent-workflow/bm5-typesafe-semantic-qualification/v1",
    "benchmark_id": spec["benchmark_id"],
...
if value.get("ready") is not True:
    ...
    raise SystemExit("BM5 readiness failed")
print("BM5 readiness: passed")
PY
...
for item in failures:
    print(
        f"BM5 visual failure: pair={item.get('pair_id')} arm={item.get('arm')} "
...
if failures:
    raise SystemExit(f"BM5 visual capture failed for {len(failures)} arm(s)")
print(f"BM5 visual capture: {value.get('complete', 0)} complete; 0 harness failures")
PY
then
  "$AW_BIN" --json benchmark live-stop "$RUN_PLAN" > "$ROOT/live-stop.json" || true
  echo "BM5 model execution is preserved. Fix the visual runtime/harness and run:" >&2
...
summary = {
    "schema": "agent-workflow/bm5-development-summary/v1",
    "run_id": plan["run_id"],
...
if summary["typesafe_qualification"]["treatment_additional_records"] != 0:
    raise SystemExit(
        "BM5 treatment unexpectedly emitted additional TypeSafe calls; "
...
echo
echo "BM5 development run complete"
echo "  root:     $ROOT"
...
echo "  public results: $RESULTS_REPO/bm5"
echo
